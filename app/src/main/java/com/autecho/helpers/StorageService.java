...
public class StorageService {
    ...
    public StorageService(Context context) {
        mContext = context;
        try {
            mClient = new MobileServiceClient("https://autecho.azure-mobile.net/", "EJGwalGeNhbONhLArapTkycXhFQXww10", mContext);
        ...
        mTableContainers.where().execute(new TableJsonQueryCallback() {
            ...
            public void onCompleted(JsonElement result, int count, Exception exception,
                ...
                for (int i = 0; i < results.size(); i ++) {
                    JsonElement item = results.get(i);
                    Map<String, String> map = new HashMap<String, String>();
                    Log.d("CONTANER NAMEEEEEEE:",item.getAsJsonObject().getAsJsonPrimitive("name").getAsString());
